...
rda
plot(rda, type="text", xlim=c(-0.5,0.5), ylim=c(-0.5,0.5)) 
#2D plot based on best model including woman as factor

##################### Adding from Zaids new analysis file
##################################### Two treatment anslysis
### Reading the data and checking the samples
...
### Choosing level for analysis (Phylum ... OTU)
level = 5
...
## redundancy analysis
mod1 <- rda(d.df~ trt.df[,1]*trt.df[,2])
mod0 <- rda(d.df ~ 1)
#Backward elimination
modb <- step(mod1, scope = formula(mod0), test = "perm")
...
rda<- rda(d.df~ trt, distance= "euclidean")
#plot(rda, type="text") #2D plot based on best model including woman as factor
plot(rda)
