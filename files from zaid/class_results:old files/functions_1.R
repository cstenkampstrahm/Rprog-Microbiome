...
taxa.split.combine.ftn = function(taxa, d.df, l = 7){
  ...
  taxa.names = strsplit(as.character(taxa[,1]),split=";")
  taxa.vt = c()
  ...
  if(l==7){
    ...
    taxa.mt = matrix(NA,nrow=length(d.df[,2]),ncol=length(levels.vt))
...
df.plot.ftn = function(d.df,col.nm ="Diversity",row.nm ="Sample",value.nm="Value"){
  rnm = row.names(d.df)
  nm = colnames(d.df)
