...
# omitted sample 7A from above, was NA for sequencing
...
### can quickly generate normalized count matrices with either the following
NormCounts <- MRcounts(MRexp_cowonly, norm=TRUE, log=FALSE)
## or
normcounts1 <- cumNormMat(MRexp_cowonly, p = cumNormStat(MRexp_cowonly))
## for more stats can do the following
...
normcounts <- MRcounts(MRexp_cowonlynormed)
nonnormedcounts <- MRcounts(MRexp_cowonly) # shows that the counts between one and 
...
## additional estimates of alpha diversity (generating these measures can be 
## found in phyloseq workflow file)
